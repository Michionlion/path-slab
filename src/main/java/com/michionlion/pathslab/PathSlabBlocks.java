...
public final class PathSlabBlocks {
    ...
    public static final Item PATH_SLAB_ITEM =
            new BlockItem(PATH_SLAB, itemProperties("dirt_path_slab"));
    ...
    private static BlockBehaviour.Properties pathProperties(String id) {
        return BlockBehaviour.Properties.ofFullCopy(Blocks.DIRT_PATH)
                ...
                .setId(
                        ResourceKey.create(
                                Registries.BLOCK,
                                Identifier.fromNamespaceAndPath(PathSlab.MOD_ID, id)));
    }
    ...
    private static void registerCreativeEntries() {
        ItemGroupEvents.MODIFY_ENTRIES_ALL.register(
                (group, entries) -> {
                    boolean hasDirtPath =
                            entries.getDisplayStacks().stream()
